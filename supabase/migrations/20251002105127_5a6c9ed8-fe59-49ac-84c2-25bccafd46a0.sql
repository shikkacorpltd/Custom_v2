-- Migration: Separate roles into dedicated table for security (Fixed)
-- This prevents privilege escalation attacks by isolating role management

-- Step 1: Create the user_roles table if not exists
...
-- Step 4: Update get_user_role function for backward compatibility
...
-- Step 5: Drop and recreate RLS policies for user_roles table
...
CREATE POLICY "School admins can update school profiles" ON public.user_profiles FOR UPDATE
USING (
...
CREATE POLICY "School admins can view school profiles" ON public.user_profiles FOR SELECT
USING (
  (SELECT school_id FROM public.user_profiles WHERE user_id = auth.uid()) = school_id
...
CREATE POLICY "Super admins can update all profiles" ON public.user_profiles FOR UPDATE
USING (public.has_role(auth.uid(), 'super_admin'))
...
CREATE POLICY "School admins can update their own school" ON public.schools FOR UPDATE
USING (
...
CREATE POLICY "Super admins can view all schools" ON public.schools FOR SELECT
USING (public.has_role(auth.uid(), 'super_admin'));
...
CREATE POLICY "School admins can update applications for their school" ON public.teacher_applications FOR UPDATE
USING (
...
CREATE POLICY "School admins can view applications for their school" ON public.teacher_applications FOR SELECT
USING (
  school_id = get_user_school(auth.uid())
  AND (public.has_role(auth.uid(), 'school_admin') OR public.has_role(auth.uid(), 'super_admin'))
);
...
CREATE POLICY "Super admins can update all applications" ON public.teacher_applications FOR UPDATE
USING (public.has_role(auth.uid(), 'super_admin'))
WITH CHECK (public.has_role(auth.uid(), 'super_admin'));

DROP POLICY IF EXISTS "Super admins can view all applications" ON public.teacher_applications;
CREATE POLICY "Super admins can view all applications" ON public.teacher_applications FOR SELECT
USING (public.has_role(auth.uid(), 'super_admin'));

-- classes policies
DROP POLICY IF EXISTS "School admins can manage classes" ON public.classes;
CREATE POLICY "School admins can manage classes" ON public.classes FOR ALL
USING (
  school_id = get_user_school(auth.uid())
  AND (public.has_role(auth.uid(), 'school_admin') OR public.has_role(auth.uid(), 'super_admin'))
);
...
CREATE POLICY "School admins can manage students" ON public.students FOR ALL
USING (
  school_id = get_user_school(auth.uid())
  AND (public.has_role(auth.uid(), 'school_admin') OR public.has_role(auth.uid(), 'super_admin'))
);
...
CREATE POLICY "School admins can manage subjects" ON public.subjects FOR ALL
USING (
  school_id = get_user_school(auth.uid())
  AND (public.has_role(auth.uid(), 'school_admin') OR public.has_role(auth.uid(), 'super_admin'))
);
...
CREATE POLICY "School admins can manage exams" ON public.exams FOR ALL
USING (
  school_id = get_user_school(auth.uid())
  AND (public.has_role(auth.uid(), 'school_admin') OR public.has_role(auth.uid(), 'super_admin'))
);
...
CREATE POLICY "School admins can manage results" ON public.exam_results FOR ALL
USING (
  school_id = get_user_school(auth.uid())
  AND (public.has_role(auth.uid(), 'school_admin') OR public.has_role(auth.uid(), 'super_admin'))
);
